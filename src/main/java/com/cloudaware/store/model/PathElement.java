...
public final class PathElement implements Serializable {
    ...
    private static final int MAX_NAME_LEMGTH = 500;

    private final String kind;
    private final Long id;
    private final String name;

    private PathElement(final String kind, final String name, final Long id) {
...
//    }
    ...
    static PathElement of(final String kind) {
        return new PathElement(kind, null, null);
    }

    public static PathElement of(final String kind, final String name) {
        checkArgument(!Strings.isNullOrEmpty(name), "name must not be empty or null");
        checkArgument(name.length() <= MAX_NAME_LEMGTH, "name must not exceed 500 characters");
        return new PathElement(kind, name, null);
    }

    public static PathElement of(final String kind, final long id) {
